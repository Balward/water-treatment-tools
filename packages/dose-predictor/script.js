...
const SEASONAL_PRESETS = {
  spring: {
    ...
    raw_fe: 0.020,      // Moderate iron levels
  ...
  fall: {
    name: 'Fall',
    icon: '🍂',
    raw_mn: 0.040,      // Moderate manganese from leaf decomposition
...
function initializeEventListeners() {
  ...
  // Form submission
  document.getElementById('parametersForm').addEventListener('submit', handleFormSubmit);
...
function validateInputs(data) {
  const ranges = {
    raw_mn: { min: 0.001, max: 0.500, name: 'Raw Manganese' },
    raw_fe: { min: 0.001, max: 0.300, name: 'Raw Iron' },
