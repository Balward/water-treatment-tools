...
function updateCoagulationSankey() {
  if (!data || data.length === 0) {
    showNotification("No data available for coagulation analysis", "warning");
    return;
  }

  // Get control values
  const timeRange = document.getElementById("sankeyTimeRange").value;
  const minFlow = parseInt(document.getElementById("sankeyMinFlow").value);
  const focusArea = document.getElementById("sankeyFocus").value;

  // Filter data based on time range
  let filteredData = [...data];
  // TODO: Implement time filtering based on timeRange

  // Categorize the data for Sankey flow
  const flowData = categorizeCoagulationData(filteredData);
  
  // Build Sankey nodes and links
  const { nodes, links } = buildCoagulationFlows(flowData, minFlow, focusArea);
  
  // Initialize or update the Sankey chart
  const chartDom = document.getElementById('coagulationSankey');
  if (coagulationSankey) {
    coagulationSankey.dispose();
  }
  coagulationSankey = echarts.init(chartDom);
    ...
    tooltip: {
      ...
      formatter: function(params) {
        if (params.dataType === 'edge') {
          const percentage = ((params.value / filteredData.length) * 100).toFixed(1);
          return `<b>${params.data.source}</b><br/>
                  ↓<br/>
                  <b>${params.data.target}</b><br/>
                  <span style="color: #00677f">Records: ${params.value} (${percentage}%)</span>`;
        } else {
          return `<b>${params.name}</b><br/>
                  <span style="color: #00677f">Total Records: ${params.value}</span>`;
        }
      }
    },
    series: [
      {
        ...
        itemStyle: {
          color: function(params) {
            ...
            if (params.name.includes('Turbidity') || params.name.includes('pH')) {
              return '#E94F37'; // Raw water conditions - coral red
            } else if (params.name.includes('Coagulant') || params.name.includes('Dose')) {
              return '#00677f'; // Coagulation strategy - deep teal  
            } else if (params.name.includes('Streaming') || params.name.includes('Current')) {
              return '#84BD00'; // Intermediate - fresh green
            } else {
              return '#2F3E46'; // Final performance - charcoal
            }
          },
          borderColor: '#fff',
          borderWidth: 1
  ...
  // Set the option and render the chart
  coagulationSankey.setOption(option);

  // Update insights panel
  updateSankeyInsights(flowData, nodes, links);

  // Make chart responsive
  window.addEventListener('resize', function() {
    if (coagulationSankey) {
      coagulationSankey.resize();
    }
  });
}

// Categorize data for coagulation flow analysis
  ...
  dataset.forEach(record => {
    ...
    if (typeof turbidity === 'number' && typeof streamingCurrent === 'number') {
      const flow = {
        ...
        // Final performance
        finalPerformance: categorizeFinalPerformance(turbidity, residualTurbidity)
      };
      
      categorizedData.push(flow);
    }
  });
...
function categorizePH(pH) {
  if (!pH || typeof pH !== 'number') return 'Unknown pH';
  if (pH < 6.5) return 'Low pH';
  if (pH < 8.5) return 'Optimal pH';
  return 'High pH';
}
...
function categorizeStreamingCurrent(current) {
  if (typeof current !== 'number') return 'No SC Data';
  const absCurrent = Math.abs(current);
  if (absCurrent < 30) return 'Excellent SC';
  if (absCurrent < 80) return 'Good SC';
  return 'Poor SC';
}

function categorizeFinalPerformance(turbidity, residual) {
  if (typeof turbidity !== 'number') return 'Unknown Performance';
  if (turbidity < 0.3) return 'Excellent';
  if (turbidity < 1.0) return 'Good';
...
function buildCoagulationFlows(flowData, minFlow, focusArea) {
  ...
  // Count flows and nodes
  flowData.forEach(flow => {
    // Define the flow path based on focus area
    let flowPath = [];
    
    if (focusArea === 'full') {
      flowPath = [
        flow.turbidityCategory,
        flow.coagulantStrategy, 
        flow.streamingCurrentPerformance,
        flow.finalPerformance
      ];
      ...
      // Focus on successful pathways
      if (flow.finalPerformance === 'Excellent' || flow.finalPerformance === 'Good') {
        flowPath = [
          flow.turbidityCategory,
          flow.coagulantStrategy,
          flow.finalPerformance
    ...
    for (let i = 0; i < flowPath.length - 1; i++) {
      const flowKey = `${flowPath[i]} → ${flowPath[i + 1]}`;
      flowCounts[flowKey] = (flowCounts[flowKey] || 0) + 1;
    }
  });
  
  // Build nodes array
  const nodes = Object.keys(nodeCounts)
    .filter(node => nodeCounts[node] >= minFlow)
    .map(name => ({
      name: name,
      value: nodeCounts[name]
    }));
  
  // Build links array  
  const links = Object.keys(flowCounts)
    .filter(flow => flowCounts[flow] >= minFlow)
    .map(flowKey => {
      const [source, target] = flowKey.split(' → ');
      return {
        source: source,
        target: target,
        value: flowCounts[flowKey]
      };
    })
    .filter(link => 
...
// Update insights panel with flow analysis
function updateSankeyInsights(flowData, nodes, links) {
  const insights = document.getElementById('sankeyInsights');
  const totalFlows = flowData.length;
  const uniqueNodes = nodes.length;
  const totalLinks = links.length;
  
  const topFlow = links.reduce((max, link) => 
    link.value > (max?.value || 0) ? link : max, null);
  
  const topNode = nodes.reduce((max, node) => 
    node.value > (max?.value || 0) ? node : max, null);
  
  insights.innerHTML = `
    <div class="sankey-stats">
      <div class="stat-item">
        <span class="stat-label">📊 Total Records</span>
        <span class="stat-value">${totalFlows.toLocaleString()}</span>
      </div>
      <div class="stat-item">
        <span class="stat-label">🔗 Flow Paths</span>
        <span class="stat-value">${totalLinks}</span>
      </div>
      <div class="stat-item">
        <span class="stat-label">📍 Process States</span>
        <span class="stat-value">${uniqueNodes}</span>
      </div>
      ${topFlow ? `
        <div class="stat-item highlight">
          <span class="stat-label">🌊 Dominant Flow</span>
          <span class="stat-value">${topFlow.source} → ${topFlow.target}</span>
          <span class="stat-detail">${topFlow.value} records (${((topFlow.value/totalFlows)*100).toFixed(1)}%)</span>
        </div>
      ` : ''}
      ${topNode ? `
        <div class="stat-item highlight">
          <span class="stat-label">⭐ Most Common State</span>
          <span class="stat-value">${topNode.name}</span>
          <span class="stat-detail">${topNode.value} records (${((topNode.value/totalFlows)*100).toFixed(1)}%)</span>
        </div>
      ` : ''}
    </div>
  `;
...
window.updateCoagulationSankey = updateCoagulationSankey;
