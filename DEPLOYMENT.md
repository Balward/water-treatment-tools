...
The application will be available at `http://your-server-ip:6969`
...
```nginx
location / {
    proxy_pass http://your-server-ip:6969;
...
The application runs on port 80 inside the container and is mapped to port 6969 on the host. To change the external port, modify the docker-compose.yml:
...
```bash
curl http://your-server-ip:6969/health
```
